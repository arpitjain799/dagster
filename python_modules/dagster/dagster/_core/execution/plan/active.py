...
class ActiveExecution:
    ...
    def get_next_step(self) -> ExecutionStep:
        ...
        steps = self.get_steps_to_execute(limit=1, register_steps=False)
        step = None
    ...
    def get_steps_to_execute(
        ...
        for step in steps:
            ...
            step_concurrency_keys = {
                key for key in self._global_concurrency_keys if key in step.tags
            }
            if register_steps and step_concurrency_keys and self._run_id:
                state = self._instance.event_log_storage.claim_concurrency_slots(
                    step_concurrency_keys, self._run_id, step.key
                )
